<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      // Try to get active training plan
      try {
...
<div>
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <!-- Race Day Countdown -->
      {#if activePlanData}
        {@const daysLeft = Math.max(0, Math.ceil((new Date(activePlanData.race_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))}
        {@const planStart = new Date(activePlanData.start_date)}
        {@const totalDays = Math.ceil((new Date(activePlanData.race_date).getTime() - planStart.getTime()) / (1000 * 60 * 60 * 24))}
        {@const elapsed = totalDays - daysLeft}
        <a href="/training" class="card p-4 mb-6 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          <div class="text-center">
            <div class="text-2xl font-bold text-primary-500">{daysLeft}</div>
            <div class="text-[10px] text-gray-400">days left</div>
          </div>
          <div class="flex-1">
            <div class="text-sm font-medium">{activePlanData.plan_display_name}</div>
            <div class="text-xs text-gray-400">Race: {format(parseISO(activePlanData.race_date), 'MMM d, yyyy')}</div>
